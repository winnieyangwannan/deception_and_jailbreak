...
for row in range(n_row):
    for col in range(n_col):
        ...
        true_label_honest = [
            data["completions"][ii]["label"] for ii in range(len(data["completions"]))
        ]
        predicted_label_lying = [
            data["completions"][ii]["predicted_label"]
            for ii in range(len(data["completions"]))
        ]

        fig_cm = plot_performance_confusion_matrix(
            true_label_honest, predicted_label_lying, save_fig=False
        )
        ...
        fig.update_xaxes(
            tickvals=[0, 1], ticktext=["False", "True"], row=row + 1, col=col + 1
        )
        fig.update_yaxes(
            tickvals=[0, 1], ticktext=["False", "True"], row=row + 1, col=col + 1
        )
...
pio.write_image(fig, save_path + os.sep + "apdx_lying_confusion_all_models.pdf")
pio.write_image(
    fig, save_path + os.sep + "apdx_lying_confusion_all_models.png", scale=6
)

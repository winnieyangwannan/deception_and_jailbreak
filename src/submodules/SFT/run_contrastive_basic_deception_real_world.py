...
def parse_arguments():
    ...
    # D:\Data\deception
    parser.add_argument("--task_name", type=str, required=False, default="real_world")
...
def main(model_path_generation, data_dir, save_dir):
    ...
    with open(
        os.path.join(
            data_dir,
            "real_world_dataset_strong_gpt4o.json",
        )
    ) as file:
        ds = json.load(file)
    ...
    message_honest = contrastive_sft.prepare_dataset_deception_real_world(
        model_name_generation, ds, "honest"
    )
    message_lying = contrastive_sft.prepare_dataset_deception_real_world(
